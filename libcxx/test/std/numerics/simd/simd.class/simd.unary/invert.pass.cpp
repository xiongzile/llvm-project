...
template <class T>
concept has_invert = requires(T v) { ~v; };
...
constexpr bool test() {
  ...
  static_assert(has_invert<stdx::vec<int>>);

  types::for_each(types::vectorizable_float_types{}, []<class T> {
    simd_utils::test_sizes([]<int N>(std::integral_constant<int, N>) { static_assert(!has_invert<stdx::vec<T, N>>); });
  });
