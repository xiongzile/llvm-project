...
template <class T>
concept has_xor = requires(T v) { v ^= v; };
...
constexpr bool test() {
  ...
  static_assert(has_xor<stdx::vec<int>>);

  types::for_each(types::vectorizable_float_types{}, []<class T> {
    simd_utils::test_sizes([]<int N>(std::integral_constant<int, N>) { static_assert(!has_xor<stdx::vec<T, N>>); });
  });
